...
# Use a fast default; can override via ENV.
...
def _get_model():
    global _model
    if _model is None:
        logger.info("Loading grounding model: %s", _MODEL_NAME)
...
def compute_grounding(draft: str, context: str) -> Dict[str, Any]:
    ...
    sims = util.cos_sim(d_emb, c_emb)  # [D, C]
...
def passes_grounding(details: Dict[str, Any]) -> bool:
    ...
    return (details.get("avg_max_sim", 0.0) >= avg_thresh) and (details.get("covered_frac", 0.0) >= covered_thresh)
